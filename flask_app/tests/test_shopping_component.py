...
from flask_app.tests.test_constants import (
    ...
    TEST_SHOP_ID_ALT,
    TEST_ITEM_ID,
    TEST_ITEM_ID_PERFUME,
    TEST_PRICE_COFFEE,
    TEST_PRICE_PERFUME_BASE,
    TEST_PRICE_PERFUME_SMALL,
    TEST_PRICE_ADJUSTMENT_SMALL,
    TEST_PRICE_ADJUSTMENT_LARGE,
    TEST_PRICE_ADJUSTMENT_NONE,
    TEST_STOCK_FULL,
    TEST_STOCK_LOW,
...
class TestProductVariants:
    ...
    def test_get_shop_items_includes_variants(self, client, monkeypatch):
        ...
        sample_items = {
            "items": [
                {
                    "item_id": TEST_ITEM_ID_PERFUME,
                    "name": "Perfume Set",
                    "base_price": TEST_PRICE_PERFUME_BASE,
                    "image_url": "https://example.com/perfume.jpg",
                    "stock_quantity": TEST_STOCK_LOW,
                    "variants": [
                        {
                            "variant_type": "Size",
                            "variant_value": "50ml",
                            "price_adjustment": TEST_PRICE_ADJUSTMENT_SMALL,
                            "final_price": TEST_PRICE_PERFUME_SMALL
                        },
                        {
                            "variant_type": "Size",
                            "variant_value": "100ml",
                            "price_adjustment": TEST_PRICE_ADJUSTMENT_NONE,
                            "final_price": TEST_PRICE_PERFUME_BASE
                        }
                    ],
                    "variant_types": ["Size"]
                }
        ...
        response = client.get(f"/shops/{TEST_SHOP_ID_ALT}/items")
        assert response.status_code == HTTP_OK
        data = response.json
        item = data["items"][0]
        assert "variants" in item
        assert "variant_types" in item
        assert len(item["variants"]) == TEST_VARIANT_COUNT
    ...
    def test_variant_final_price_calculation(self):
        ...
        base_price = TEST_PRICE_PERFUME_BASE
        price_adjustment = TEST_PRICE_ADJUSTMENT_SMALL
        expected_final = TEST_PRICE_PERFUME_SMALL
...
class TestStockQuantity:
    ...
    def test_get_shop_items_includes_stock_quantity(self, client, monkeypatch):
        ...
        assert response.json["items"][0]["stock_quantity"] == TEST_STOCK_FULL
    ...
    def test_stock_quantity_is_integer(self, client, monkeypatch):
        ...
        response = client.get(f"/shops/{TEST_SHOP_ID}/items")
        assert response.status_code == HTTP_OK
        stock_qty = response.json["items"][0]["stock_quantity"]
        assert isinstance(stock_qty, int)
...
class TestTimeParsing:
    ...
    def test_parse_single_digit_hour(self):
        ...
        assert parsed_time.hour == TEST_OPEN_HOUR
    ...
    def test_parse_evening_time(self):
        ...
        assert parsed_time.hour == TEST_CLOSE_HOUR
        assert parsed_time.minute == 0
    ...
    def test_time_comparison_open_hours(self):
        ...
        open_time = time(TEST_OPEN_HOUR, 0)
        close_time = time(TEST_CLOSE_HOUR, 0)

        # Test time during open hours (noon)
        current_time = time(TEST_NOON_HOUR, 0)
        is_open = open_time <= current_time <= close_time
        assert is_open is True

        # Test time before open (5am)
        current_time = time(TEST_EARLY_HOUR, 0)
        is_open = open_time <= current_time <= close_time
        assert is_open is False

        # Test time after close (11pm)
        current_time = time(TEST_LATE_HOUR, 0)
        is_open = open_time <= current_time <= close_time
        assert is_open is False
...
class TestBookingWithVariants:
    ...
    def test_create_booking_with_variants_calculates_price(self):
        ...
        expected = (TEST_PRICE_PERFUME_BASE + TEST_PRICE_ADJUSTMENT_SMALL) * TEST_QUANTITY_SINGLE
    ...
    def test_create_booking_with_multiple_quantity_and_variants(self):
        ...
        total = calculate_total_price(base_price, quantity, variants)
        expected = TEST_PRICE_PERFUME_BASE * TEST_QUANTITY_DOUBLE
        assert round(total, 2) == round(expected, 2)
        ...
        variants = [
            {
                "variant_type": "Size",
                "variant_value": "200ml",
                "price_adjustment": TEST_PRICE_ADJUSTMENT_LARGE
            }
        ]

        total = calculate_total_price(base_price, quantity, variants)
        expected = TEST_PRICE_PERFUME_BASE + TEST_PRICE_ADJUSTMENT_LARGE
        assert round(total, 2) == round(expected, 2)